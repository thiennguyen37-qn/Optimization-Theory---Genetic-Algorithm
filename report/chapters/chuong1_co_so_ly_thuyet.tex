...
truyền (Genetic Algorithm -- GA). Nội dung chương được tổ chức thành bốn
...
\begin{itemize}
    ...
    hàng (Travelling Salesman Problem -- TSP) hay bài toán lập lịch, số
...
Tiến hóa} (Evolutionary Algorithms -- EA) là đại diện tiêu biểu nhất của
...
\begin{itemize}
    \item \textbf{Thuật toán Di truyền} (Genetic Algorithm -- GA): đặt
...
\begin{itemize}
    ...
    Fittest''}): những cá thể có khả năng thích nghi tốt hơn với môi
...
phát từ một quần thể các giải pháp ngẫu nhiên; trong suốt quá trình tiến
hóa, các giải pháp có chất lượng tốt hơn sẽ có nhiều cơ hội được chọn
...
giải pháp cần được biểu diễn dưới dạng một ``nhiễm sắc thể'' mà các
...
\begin{itemize}
    ...
    \item \textbf{Mã hóa hoán vị} (Permutation Encoding): giải pháp là
    ...
    nơi lời giải về bản chất là một hoán vị.
    \item \textbf{Mã hóa dựa trên cấu trúc} (Tree/Graph Encoding):
    giải pháp được biểu diễn dưới dạng cây biểu thức hoặc đồ thị, chủ
...
\begin{itemize}
    ...
    \item \textbf{Thách đấu} (Tournament Selection): chọn ngẫu nhiên $k$
    ...
    theo.
    \item \textbf{Chọn lọc theo thứ hạng} (Rank-based Selection): cá thể
...
\begin{itemize}
    ...
    cha mẹ.
    \item \textbf{Lai ghép đa điểm} (Multi-point Crossover): là dạng
    tổng quát của lai ghép đơn điểm với nhiều vị trí cắt; các đoạn xen
    kẽ giữa hai cha mẹ được hoán đổi cho nhau.
    \item \textbf{Lai ghép đồng nhất} (Uniform Crossover): giá trị của
...
trộn (Blend Crossover, BLX-$\alpha$), cũng chính là toán tử được sử
...
\begin{itemize}
    \item \textbf{PMX} (Partially Matched Crossover): hoán đổi một đoạn
    ...
    \item \textbf{OX} (Order Crossover): giữ nguyên một đoạn con từ cha
...
\begin{itemize}
    \item \textbf{Đột biến đảo bit} (Bit-flip Mutation): áp dụng cho mã
    hóa nhị phân; mỗi bit được đảo giá trị ($0 \leftrightarrow 1$) độc
    lập với xác suất $p_m$.
    \item \textbf{Đột biến Gauss} (Gaussian Mutation): áp dụng cho mã
    ...
    \item \textbf{Đột biến hoán đổi, đảo ngược và chèn} (Swap,
    Inversion, Insertion): áp dụng cho mã hóa hoán vị; lần lượt là hoán
...
\begin{itemize}
    ...
    \item \textbf{Độ trễ hội tụ} (stagnation): thuật toán dừng khi độ
...
Lập trình Di truyền (Genetic Programming -- GP) do John Koza phát triển
...
định trong khuôn dạng lời giải đã cho trước, GP tiến hóa trực tiếp
...
dự đoán, tiêu biểu là sai số bình phương trung bình (MSE) giữa giá trị
cây tính được và giá trị $y$ thực tế. Ngoài ra, một số hạng phạt tỉ lệ
với kích thước cây (\textit{parsimony pressure}) thường được cộng thêm,
nhằm hạn chế hiện tượng cây phát triển không kiểm soát (\textit{bloat})
...
\begin{itemize}
    ...
    \item \textbf{Đột biến thay thế cây con} (Subtree Mutation): chọn
