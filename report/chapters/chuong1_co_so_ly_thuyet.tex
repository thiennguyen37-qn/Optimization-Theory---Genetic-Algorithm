...
Hình~\ref{fig:ga-crossover-pmx} minh họa lai ghép khớp từng phần
(Partially Matched Crossover). Đoạn từ vị trí $3$ đến $5$ (tô cam) được
hoán đổi trực tiếp giữa hai cha mẹ, đồng thời tạo ra một ánh xạ tương
ứng từng cặp giá trị trong đoạn đó. Đối với các vị trí còn lại (tô
xanh lá), giá trị gốc của cha mẹ được giữ nguyên nếu không gây trùng
lặp với đoạn vừa hoán đổi; nếu trùng lặp, giá trị được thay thế theo
ánh xạ đã tạo cho đến khi thu được một giá trị chưa xuất hiện trong cá
thể con.
...
\node[font=\small\itshape] at (2.8,1.3) {Mã hóa hoán vị -- Lai ghép khớp từng phần (Partially Matched Crossover)};
...
\end{tikzpicture}
\caption{Ví dụ lai ghép khớp từng phần (Partially Matched Crossover)
trên mã hóa hoán vị của $8$ phần tử. Đoạn hoán đổi trực tiếp giữa hai
cha mẹ (vị trí $3$ đến $5$) tô cam; các vị trí còn lại của cá thể con,
giữ nguyên hoặc được sửa theo ánh xạ để tránh trùng lặp, tô xanh lá.}
\label{fig:ga-crossover-pmx}
\end{figure}

Hình~\ref{fig:ga-crossover-ox} minh họa lai ghép theo thứ tự (Order
Crossover) trên cùng cặp cha mẹ -- toán tử được sử dụng trong phần cài
đặt thực nghiệm cho bài toán người du lịch tại
Chương~\ref{chap:ung-dung}. Đoạn từ vị trí $3$ đến $5$ (tô cam) được
giữ nguyên từ cha mẹ tương ứng để tạo thành cá thể con cùng hàng; các
vị trí còn lại của cá thể con (tô xanh lá) được điền theo đúng thứ tự
xuất hiện của các phần tử chưa dùng trong cha mẹ còn lại.
...
\node[font=\small\itshape] at (2.8,1.3) {Mã hóa hoán vị -- Lai ghép theo thứ tự (Order Crossover)};
...
\end{tikzpicture}
\caption{Ví dụ lai ghép theo thứ tự (Order Crossover) trên mã hóa hoán
vị của $8$ phần tử. Đoạn giữ nguyên từ cha mẹ tương ứng (vị trí $3$
đến $5$) tô cam; các vị trí còn lại của cá thể con, điền theo thứ tự
xuất hiện trong cha mẹ kia (bỏ qua phần tử đã dùng), tô xanh lá.}
\label{fig:ga-crossover-ox}
\end{figure}

Hình~\ref{fig:ga-crossover-cx} minh họa lai ghép theo chu trình (Cycle
Crossover) trên cùng cặp cha mẹ. Ba chu trình được xác định giữa vị
trí phần tử của hai cha mẹ (tô cam, xanh lá và xanh dương); mỗi cá thể
con được tạo ra bằng cách giữ nguyên các chu trình này nhưng luân
phiên lấy giá trị từ mỗi cha mẹ.
...
\node[font=\small\itshape] at (2.8,1.3) {Mã hóa hoán vị -- Lai ghép theo chu trình (Cycle Crossover)};
...
\end{tikzpicture}
\caption{Ví dụ lai ghép theo chu trình (Cycle Crossover) trên mã hóa
hoán vị của $8$ phần tử. Ba chu trình giữa vị trí phần tử của hai cha
mẹ được tô cam, xanh lá và xanh dương. Con 1 lấy chu trình cam và xanh
dương từ Cha mẹ 1, chu trình xanh lá từ Cha mẹ 2; Con 2 lấy ngược lại.}
