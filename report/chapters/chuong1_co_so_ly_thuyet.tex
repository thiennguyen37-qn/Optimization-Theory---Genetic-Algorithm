...
thuật toán và điều kiện dừng. Phần cuối trình bày Lập trình Di truyền
(Genetic Programming – GP), một phương pháp phát triển từ các nguyên
lý của Thuật toán Di truyền (GA), cho phép tiến hóa trực tiếp cấu trúc
của chương trình hoặc biểu thức để tìm ra lời giải phù hợp.
...
\begin{itemize}
    ...
    nhiều lớp bài toán tối ưu tổ hợp, tiêu biểu như bài toán người du
    lịch hay bài toán lập lịch, số
...
trình Di truyền được dùng cho bài toán hồi quy symbolic. Khung vận hành
...
\begin{itemize}
    ...
    \item \textbf{Mã hóa số thực}: nghiệm được biểu diễn dưới dạng một
    dãy số thực, chẳng hạn $[3.14,\, -0.05,\, 12.8]$, trong đó mỗi phần
    tử tương ứng trực tiếp với một biến quyết định. Phương pháp mã hóa
    này đặc biệt phù hợp với các bài toán tối ưu liên tục nhiều biến,
    đồng thời tránh được sai số do giới hạn độ chính xác khi biểu diễn
    nghiệm bằng mã nhị phân.
...
\textbf{Hàm độ thích nghi} được sử dụng để đánh giá chất lượng của
từng cá thể trong quần thể và làm cơ sở cho quá trình chọn lọc. Đối
với bài toán không có ràng buộc, hàm độ thích nghi thường được xây
dựng dựa trên hàm mục tiêu. Với bài toán cực tiểu hóa, nghiệm tốt là
nghiệm có giá trị hàm mục tiêu càng nhỏ nhưng do GA thường ưu tiên các
cá thể có độ thích nghi càng lớn, hàm mục tiêu có thể được biến đổi,
chẳng hạn bằng cách đổi dấu, để giá trị độ thích nghi tăng tương ứng
với mức độ tốt của nghiệm.

Đối với bài toán có ràng buộc, trọng tâm của tiểu luận này,
...
đề xuất, không còn dựa vào một hệ số phạt cố định.

\textbf{Quy tắc khả thi của Deb} [3] thay hàm phạt bằng một quy tắc so
sánh trực tiếp giữa hai cá thể, dựa trên ba trường hợp: (i) giữa hai
cá thể đều khả thi, cá thể có giá trị hàm mục tiêu tốt hơn luôn được
ưu tiên; (ii) giữa một cá thể khả thi và một cá thể không khả thi, cá
thể khả thi luôn được ưu tiên bất kể giá trị hàm mục tiêu; (iii) giữa
hai cá thể đều không khả thi, cá thể có tổng mức vi phạm ràng buộc nhỏ
hơn được ưu tiên. Quy tắc này loại bỏ hoàn toàn nhu cầu xác định hệ số
phạt, vì việc so sánh không còn cộng gộp mức vi phạm vào hàm mục tiêu.

\textbf{Ngưỡng $\varepsilon$ giảm dần} do Takahama và Sakai đề xuất [4]
không yêu cầu mức vi phạm bằng $0$ ngay từ đầu. Thay vào đó, cá thể
được xem là khả thi khi tổng mức vi phạm không vượt quá ngưỡng
$\varepsilon(t)$. Ngưỡng này bắt đầu ở giá trị dương, cho phép quần
thể khám phá cả vùng không khả thi, rồi giảm dần về $0$ tại thế hệ
$T_c$, từ đó hướng quần thể hội tụ vào miền khả thi.

\subsection*{Các Phép chọn lọc}
\label{subsec:chon-loc}

Toán tử chọn lọc quyết định những cá thể trong quần thể hiện tại được
lựa chọn làm cha mẹ để tạo ra thế hệ tiếp theo. Về nguyên tắc, các cá
thể có độ thích nghi cao sẽ có cơ hội được lựa chọn lớn hơn, qua đó
làm tăng khả năng các đặc điểm tốt được duy trì và truyền sang thế hệ
sau. Ba phương pháp chọn lọc phổ biến được trình bày dưới đây.

\begin{itemize}
    \item \textbf{Vòng quay Roulette}: Xác
    suất chọn cá thể $i$ tỉ lệ thuận với độ thích nghi của cá thể đó so
    với tổng độ thích nghi của toàn bộ quần thể:
    \begin{equation}
    P_i = \frac{f_i}{\sum_{j=1}^{N} f_j}.
    \label{eq:roulette}
    \end{equation}
    Hạn chế của phương pháp này xuất hiện khi một cá thể có độ thích
    nghi vượt trội so với phần còn lại. Khi đó, cá thể này chiếm phần
    lớn xác suất được lựa chọn, làm tăng áp lực chọn lọc và có thể
    khiến tính đa dạng của quần thể giảm nhanh.
    \item \textbf{Giải đấu}: Chọn ngẫu nhiên $k$
    cá thể từ quần thể, sau đó chọn cá thể tốt nhất trong nhóm làm cha
    mẹ. Tham số $k$, được gọi là kích thước giải đấu, là một yếu tố
    điều khiển áp lực chọn lọc: $k$ càng lớn thì cá thể tốt càng có
    lợi thế trong quá trình lựa chọn, quần thể thường hội tụ nhanh
    hơn, nhưng đồng thời nguy cơ mất đa dạng và hội tụ sớm cũng tăng.
    \item \textbf{Chọn lọc theo thứ hạng}: Các cá
    thể được sắp xếp theo độ thích nghi và việc lựa chọn được thực
    hiện dựa trên thứ hạng thay vì giá trị độ thích nghi tuyệt đối.
    Nhờ đó, sự chênh lệch quá lớn giữa các giá trị độ thích nghi không
    khiến một cá thể chiếm ưu thế quá mức về xác suất được lựa chọn
    như trong phương pháp vòng quay Roulette.
\end{itemize}
...
vị trí $5$ đến $8$ được hoán đổi giữa hai cha mẹ để tạo ra hai cá thể
con. Với mã hóa số thực, hai cá
...
\node[align=center, font=\small] at (1.3,-7.8) {$\Downarrow$ \eqref{eq:blend-crossover}};

\node[gene, minimum width=13mm] at (0.5,-8.7) {$2.5$};
\node[gene, minimum width=13mm] at (2.15,-8.7) {$-0.25$};
\node[anchor=east, font=\small] at (-0.9,-8.7) {Con 1};

\node[gene, minimum width=13mm] at (0.5,-9.6) {$1.5$};
\node[gene, minimum width=13mm] at (2.15,-9.6) {$1.25$};
\node[anchor=east, font=\small] at (-0.9,-9.6) {Con 2};

\end{tikzpicture}
\caption{Ví dụ lai ghép trong GA trên hai cách mã hóa. Trên: mã hóa
nhị phân với lai ghép đơn điểm. Dưới: mã hóa số thực với lai ghép trộn
theo \eqref{eq:blend-crossover}, $\alpha = 0.75$.}
...
Để tiện so sánh, cả ba ví dụ dưới đây đều xuất phát từ cùng một cặp cha
mẹ là hai hoán vị của $8$ phần tử.

Hình~\ref{fig:ga-crossover-pmx} minh họa lai ghép khớp từng phần. Đoạn
từ vị trí $3$ đến $5$ được hoán đổi trực tiếp giữa hai cha mẹ, đồng
thời tạo ra một ánh xạ tương ứng từng cặp giá trị trong đoạn đó. Đối
với các vị trí còn lại, giá trị gốc của cha mẹ được giữ nguyên nếu
không gây trùng lặp với đoạn vừa hoán đổi; nếu trùng lặp, giá trị được
thay thế theo ánh xạ đã tạo cho đến khi thu được một giá trị chưa xuất
hiện trong cá thể con.
...
\node[font=\small\itshape] at (2.8,1.3) {Mã hóa hoán vị -- Lai ghép khớp từng phần};

\node[gene] at (0.0,0) {1};
\node[gene] at (0.8,0) {2};
\node[gene,cut] at (1.6,0) {3};
\node[gene,cut] at (2.4,0) {4};
\node[gene,cut] at (3.2,0) {5};
\node[gene] at (4.0,0) {6};
\node[gene] at (4.8,0) {7};
\node[gene] at (5.6,0) {8};
\node[anchor=east, font=\small] at (-0.5,0) {Cha mẹ 1};

\node[gene] at (0.0,-0.9) {3};
\node[gene] at (0.8,-0.9) {7};
\node[gene,cut] at (1.6,-0.9) {1};
\node[gene,cut] at (2.4,-0.9) {5};
\node[gene,cut] at (3.2,-0.9) {8};
\node[gene] at (4.0,-0.9) {2};
\node[gene] at (4.8,-0.9) {6};
\node[gene] at (5.6,-0.9) {4};
\node[anchor=east, font=\small] at (-0.5,-0.9) {Cha mẹ 2};

\draw[dashed] (1.2,0.5) -- (1.2,-1.4);
\draw[dashed] (3.6,0.5) -- (3.6,-1.4);

\node[align=center, font=\small] at (2.8,-1.9) {$\Downarrow$ hoán đổi đoạn vị trí 3--5, sửa trùng lặp theo ánh xạ};

\node[gene,graft] at (0.0,-2.8) {3};
\node[gene,graft] at (0.8,-2.8) {2};
\node[gene,cut] at (1.6,-2.8) {1};
\node[gene,cut] at (2.4,-2.8) {5};
\node[gene,cut] at (3.2,-2.8) {8};
\node[gene,graft] at (4.0,-2.8) {6};
\node[gene,graft] at (4.8,-2.8) {7};
\node[gene,graft] at (5.6,-2.8) {4};
\node[anchor=east, font=\small] at (-0.5,-2.8) {Con 1};

\node[gene,graft] at (0.0,-3.7) {1};
\node[gene,graft] at (0.8,-3.7) {7};
\node[gene,cut] at (1.6,-3.7) {3};
\node[gene,cut] at (2.4,-3.7) {4};
\node[gene,cut] at (3.2,-3.7) {5};
\node[gene,graft] at (4.0,-3.7) {2};
\node[gene,graft] at (4.8,-3.7) {6};
\node[gene,graft] at (5.6,-3.7) {8};
\node[anchor=east, font=\small] at (-0.5,-3.7) {Con 2};

\end{tikzpicture}
\caption{Ví dụ lai ghép khớp từng phần trên mã hóa hoán vị của $8$
phần tử. Đoạn hoán đổi trực tiếp giữa hai cha mẹ (vị trí $3$ đến $5$);
các vị trí còn lại của cá thể con, giữ nguyên hoặc được sửa theo ánh
xạ để tránh trùng lặp.}
\label{fig:ga-crossover-pmx}
\end{figure}

Hình~\ref{fig:ga-crossover-ox} minh họa lai ghép theo thứ tự trên cùng
cặp cha mẹ -- toán tử được sử dụng trong phần cài
đặt thực nghiệm cho bài toán người du lịch tại
Chương~\ref{chap:ung-dung}. Đoạn từ vị trí $3$ đến $5$ được giữ nguyên
từ cha mẹ tương ứng để tạo thành cá thể con cùng hàng; các vị trí còn
lại của cá thể con được điền theo đúng thứ tự xuất hiện của các phần
tử chưa dùng trong cha mẹ còn lại.
...
\node[font=\small\itshape] at (2.8,1.3) {Mã hóa hoán vị -- Lai ghép theo thứ tự};
...
\node[align=center, font=\small] at (2.8,-1.9) {$\Downarrow$ giữ đoạn vị trí 3--5, điền phần còn lại theo thứ tự ở cha mẹ kia};

\node[gene,graft] at (0.0,-2.8) {7};
\node[gene,graft] at (0.8,-2.8) {1};
\node[gene,cut] at (1.6,-2.8) {3};
\node[gene,cut] at (2.4,-2.8) {4};
\node[gene,cut] at (3.2,-2.8) {5};
\node[gene,graft] at (4.0,-2.8) {8};
\node[gene,graft] at (4.8,-2.8) {2};
\node[gene,graft] at (5.6,-2.8) {6};
\node[anchor=east, font=\small] at (-0.5,-2.8) {Con 1};

\node[gene,graft] at (0.0,-3.7) {2};
\node[gene,graft] at (0.8,-3.7) {3};
\node[gene,cut] at (1.6,-3.7) {1};
\node[gene,cut] at (2.4,-3.7) {5};
\node[gene,cut] at (3.2,-3.7) {8};
\node[gene,graft] at (4.0,-3.7) {4};
\node[gene,graft] at (4.8,-3.7) {6};
\node[gene,graft] at (5.6,-3.7) {7};
\node[anchor=east, font=\small] at (-0.5,-3.7) {Con 2};

\end{tikzpicture}
\caption{Ví dụ lai ghép theo thứ tự trên mã hóa hoán vị của $8$ phần
tử. Đoạn giữ nguyên từ cha mẹ tương ứng (vị trí $3$ đến $5$); các vị
trí còn lại của cá thể con, điền theo thứ tự xuất hiện trong cha mẹ
kia (bỏ qua phần tử đã dùng).}
\label{fig:ga-crossover-ox}
\end{figure}

Hình~\ref{fig:ga-crossover-cx} minh họa lai ghép theo chu trình trên
cùng cặp cha mẹ. Ba chu trình được xác định giữa vị trí phần tử của
hai cha mẹ; mỗi cá thể con được tạo ra bằng cách giữ nguyên các chu
trình này nhưng luân phiên lấy giá trị từ mỗi cha mẹ.

\begin{figure}[H]
\centering
\begin{tikzpicture}[
    gene/.style={rectangle, draw, minimum width=7mm, minimum height=7mm, inner sep=0pt, font=\small},
    cyca/.style={fill=orange!25, draw=orange!70!black, thick},
    cycb/.style={fill=green!20, draw=green!50!black, thick},
    cycc/.style={fill=blue!15, draw=blue!55!black, thick}
]

\node[font=\small\itshape] at (2.8,1.3) {Mã hóa hoán vị -- Lai ghép theo chu trình};
\node[font=\scriptsize] at (2.8,0.85) {Chu trình 1: cam \quad\quad Chu trình 2: xanh lá \quad\quad Chu trình 3: xanh dương};

\node[gene,cyca] at (0.0,0) {1};
\node[gene,cycb] at (0.8,0) {2};
\node[gene,cyca] at (1.6,0) {3};
\node[gene,cycc] at (2.4,0) {4};
\node[gene,cycc] at (3.2,0) {5};
\node[gene,cycb] at (4.0,0) {6};
\node[gene,cycb] at (4.8,0) {7};
\node[gene,cycc] at (5.6,0) {8};
\node[anchor=east, font=\small] at (-0.5,0) {Cha mẹ 1};

\node[gene,cyca] at (0.0,-0.9) {3};
\node[gene,cycb] at (0.8,-0.9) {7};
\node[gene,cyca] at (1.6,-0.9) {1};
\node[gene,cycc] at (2.4,-0.9) {5};
\node[gene,cycc] at (3.2,-0.9) {8};
\node[gene,cycb] at (4.0,-0.9) {2};
\node[gene,cycb] at (4.8,-0.9) {6};
\node[gene,cycc] at (5.6,-0.9) {4};
\node[anchor=east, font=\small] at (-0.5,-0.9) {Cha mẹ 2};

\node[align=center, font=\small] at (2.8,-1.6) {$\Downarrow$ luân phiên lấy chu trình từ mỗi cha mẹ};

\node[gene,cyca] at (0.0,-2.5) {1};
\node[gene,cycb] at (0.8,-2.5) {7};
\node[gene,cyca] at (1.6,-2.5) {3};
\node[gene,cycc] at (2.4,-2.5) {4};
\node[gene,cycc] at (3.2,-2.5) {5};
\node[gene,cycb] at (4.0,-2.5) {2};
\node[gene,cycb] at (4.8,-2.5) {6};
\node[gene,cycc] at (5.6,-2.5) {8};
\node[anchor=east, font=\small] at (-0.5,-2.5) {Con 1};

\node[gene,cyca] at (0.0,-3.4) {3};
\node[gene,cycb] at (0.8,-3.4) {2};
\node[gene,cyca] at (1.6,-3.4) {1};
\node[gene,cycc] at (2.4,-3.4) {5};
\node[gene,cycc] at (3.2,-3.4) {8};
\node[gene,cycb] at (4.0,-3.4) {6};
\node[gene,cycb] at (4.8,-3.4) {7};
\node[gene,cycc] at (5.6,-3.4) {4};
\node[anchor=east, font=\small] at (-0.5,-3.4) {Con 2};

\end{tikzpicture}
\caption{Ví dụ lai ghép theo chu trình trên mã hóa hoán vị của $8$
phần tử, với ba chu trình được xác định giữa vị trí phần tử của hai
cha mẹ. Con 1 lấy chu trình 1 và 3 từ Cha mẹ 1, chu trình 2 từ Cha mẹ
2; Con 2 lấy ngược lại.}
\label{fig:ga-crossover-cx}
\end{figure}
...
phân, gen tại vị trí $3$ được đảo giá trị từ $1$ thành $0$. Với mã hóa
số thực, giá trị tại vị trí $2$ được cộng thêm một nhiễu Gauss theo
\eqref{eq:dot-bien-gauss}, ở đây lấy mẫu được $\mathcal{N}(0,\sigma^2)
= -0.4$, cho ra giá trị mới $-1.4$.
...
Do các quá trình chọn lọc, lai ghép và đột biến có thể làm thay đổi hoặc loại bỏ
các cá thể tốt, GA thông thường không đảm bảo cá thể tốt nhất của thế hệ hiện
tại được duy trì nguyên vẹn ở thế hệ kế tiếp. Vì vậy, độ thích nghi tốt nhất của
quần thể có thể giảm giữa hai thế hệ liên tiếp. \textbf{Chiến lược Elitism}, do
De Jong đề xuất [5], được sử dụng để khắc phục vấn đề này bằng cách giữ lại $k$
cá thể có độ thích nghi cao nhất của thế hệ hiện tại và chuyển trực tiếp sang
thế hệ kế tiếp mà không thực hiện lai ghép hay đột biến.

Gọi $E_t$ là tập gồm $k$ cá thể ưu tú của thế hệ $t$. Khi đó, các cá thể này
được đưa trực tiếp vào thế hệ $t+1$:

\[
E_t \subseteq P_{t+1}, \qquad |E_t| = k.
\]

Nhờ đó, nếu bài toán là \textbf{cực đại hóa độ thích nghi}, độ thích nghi tốt
nhất của quần thể không thể giảm qua các thế hệ:

\[
\boxed{
f_{\mathrm{best}}(t+1) \geq f_{\mathrm{best}}(t)
}
\]

Đây là đặc tính quan trọng nhất của Elitism, đảm bảo rằng một nghiệm tốt đã
tìm được sẽ không bị mất do các phép toán tiến hóa ở các thế hệ sau.

Kích thước $k$ cần được lựa chọn phù hợp. Nếu $k$ quá nhỏ, số lượng cá thể
được bảo toàn trực tiếp ít, nên tác động của Elitism đối với việc duy trì các
nghiệm tốt bị hạn chế. Ngược lại, nếu $k$ quá lớn, một phần lớn quần thể được
giữ nguyên qua các thế hệ, làm giảm khả năng khám phá những vùng mới của
không gian tìm kiếm, đồng thời có thể làm giảm tính đa dạng và khiến quần thể
hội tụ sớm.
...
\node[box, below=of evalc] (update) {Cập nhật quần thể $P(t+1)$};
...
\While{điều kiện dừng chưa thỏa mãn}
    ...
    \State Thực hiện đột biến trên $C(t)$, tạo tập con $C'(t)$
    \State Đánh giá độ thích nghi của $C'(t)$
    \State $P(t) \gets$ tạo quần thể mới từ $C'(t)$, giữ lại $k$ cá thể ưu tú
\EndWhile
...
Một thuật toán tiến hóa cần có tiêu chí dừng rõ ràng để xác định thời điểm
kết thúc quá trình tiến hóa. Trong thực tế, thay vì chỉ sử dụng một tiêu chí
duy nhất, nhiều tiêu chí dừng thường được kết hợp để đánh giá cả số lần lặp,
mức độ cải thiện của nghiệm và sự hội tụ của quần thể. Ba tiêu chí dừng phổ
biến được trình bày dưới đây.

\begin{itemize}
    \item \textbf{Số thế hệ tối đa $G_{\max}$:} đây là tiêu chí đơn giản, đồng thời cho phép kiểm soát trực tiếp số lần lặp và tài nguyên tính toán. Giá trị $G_{\max}$ cần được lựa chọn phù hợp với bài toán: nếu $G_{\max}$ quá nhỏ, thuật toán có thể chưa có đủ thời gian để đạt được nghiệm có chất lượng tốt; ngược lại, nếu quá lớn, thuật toán có thể tiếp tục thực hiện nhiều thế hệ dù mức độ cải thiện đã không còn đáng kể.

    \item \textbf{Ngưỡng độ thích nghi mục tiêu:} thuật toán có thể kết thúc khi tìm được một cá thể có độ thích nghi đạt hoặc vượt qua một ngưỡng xác định trước. Tiêu chí này phù hợp khi có thể xác định trước mức chất lượng nghiệm mong muốn hoặc có một giá trị tham chiếu để làm cơ sở đánh giá.

    \item \textbf{Độ trễ hội tụ:} thuật toán có thể kết thúc khi độ thích nghi của cá thể tốt nhất không cải thiện đáng kể trong $K$ thế hệ liên tiếp. Tiêu chí này dựa trên quan sát về mức độ tiến bộ của quá trình tiến hóa. Tuy nhiên, nếu $K$ được đặt quá nhỏ, thuật toán có thể kết thúc trong một giai đoạn mà mức cải thiện tạm thời thấp, trong khi quá trình tìm kiếm vẫn còn khả năng tiếp tục cải thiện nghiệm ở các thế hệ sau.
\end{itemize}
...
Lập trình Di truyền (GP) được John Koza phát triển từ cuối thập niên 1980
và được hệ thống hóa trong các công trình của ông, trong đó có công
trình năm 1994 [6]. GP phát triển từ các nguyên lý của Thuật toán Di
truyền (GA), nhưng có một điểm khác biệt quan trọng về cách biểu diễn
nghiệm. Trong GA, một cá thể thường là một chuỗi giá trị dùng để biểu
diễn một nghiệm cụ thể, chẳng hạn một dãy số thực hoặc một hoán vị.
Trong khi đó, một cá thể trong GP là một chương trình hoặc một biểu
thức, và chương trình này có thể được thực thi để tạo ra kết quả cho
bài toán. Nhờ vậy, GP có thể được sử dụng để tự động tìm ra chương
trình hoặc biểu thức phù hợp với yêu cầu của bài toán mà không cần
xác định trước chính xác cấu trúc của lời giải.

\subsection*{Mã hóa dạng cây biểu thức}

Mỗi cá thể trong GP là một cây biểu thức. Mỗi nút trung gian
(internal node) của cây là một toán tử được chọn từ một \textit{tập
toán tử} cho trước, chẳng hạn các
...
Việc thiết kế tập toán tử và tập biến, hằng số cần thỏa mãn hai tính
chất cơ bản:
\begin{itemize}
...
Một trong số những ứng dụng của GP là bài toán \textbf{hồi quy symbolic}
...
Một vấn đề đặc trưng trong GP là hiện tượng \textbf{bloat}, trong đó
kích thước của các cây chương trình có xu hướng tăng qua các thế hệ mà
không mang lại sự cải thiện tương xứng về độ chính xác của nghiệm. Một
trong những nguyên nhân được đề xuất là sự tích lũy của các cây con
trung tính, tức là những phần của chương trình có thể thay đổi nhưng
không làm thay đổi đáng kể kết quả đầu ra. Ví dụ, trong một biểu thức,
các thành phần như $+0$ hoặc $\times 1$ không làm thay đổi giá trị của
biểu thức nhưng vẫn làm tăng kích thước cây. Do những thành phần này
không làm giảm độ thích nghi, chúng có thể được duy trì và tiếp tục
tích lũy qua nhiều thế hệ. Hiện tượng bloat làm tăng chi phí tính toán
khi đánh giá các cá thể, khiến chương trình trở nên phức tạp và khó
diễn giải hơn, đồng thời có thể làm tăng nguy cơ quá khớp. Một cách phổ
biến để hạn chế bloat là đưa kích thước hoặc độ phức tạp của cây vào
quá trình đánh giá, chẳng hạn bằng cách thêm một \textbf{số hạng phạt}
vào hàm thích nghi đối với những cây có kích thước lớn:
\begin{equation}
    \mathrm{fitness}(T) = \mathrm{MSE}(T) + \lambda \cdot |T|,
    \label{eq:parsimony}
\end{equation}
trong đó $|T|$ biểu thị kích thước của cây chương trình $T$, thường
được xác định bằng số lượng nút trong cây. Thành phần $\lambda \cdot
|T|$ đóng vai trò là một khoản phạt đối với những cây có kích thước
lớn, qua đó khuyến khích GP ưu tiên các chương trình có cấu trúc đơn
giản và gọn nhẹ. Tham số $\lambda > 0$ kiểm soát mức độ của khoản phạt
này, từ đó điều chỉnh sự đánh đổi giữa độ chính xác và độ phức tạp của
chương trình. Khi $\lambda$ nhỏ, quá trình tiến hóa chủ yếu tập trung
vào việc giảm sai số MSE; ngược lại, khi $\lambda$ lớn, các cây có
kích thước lớn sẽ chịu mức phạt cao hơn và do đó ít được ưu tiên hơn.
Nhờ vậy, việc bổ sung thành phần phạt kích thước cây giúp hạn chế sự
gia tăng không cần thiết của cấu trúc chương trình và góp phần kiểm
soát hiện tượng bloat. Các biện pháp bổ sung gồm
...
\begin{itemize}
    ...
    thể gồm lai ghép bất đối xứng (cây con thứ nhất lấy từ nút trung
    gian, cây con thứ hai từ nút lá) và lai ghép tự thân (cắt và dán trên
...
Điều kiện dừng thường là số thế hệ tối đa kết hợp
với ngưỡng sai số chấp nhận được.

Hình~\ref{fig:gp-crossover} minh họa phép lai ghép trao đổi cây con
giữa hai cây cha mẹ $(x+1)^2$ và $(x-1)x$ với tập toán tử
$\{+,-,\times\}$.
...
Hình~\ref{fig:gp-mutation} minh họa ba toán tử đột biến trên cây đơn
giản $x+3$ (và, với đột biến rút gọn, trên cây lớn hơn
$(x+3)\times x$).
...
gọn minh họa trên cây lớn hơn $(x+3)\times x$): (a)--(b) đột biến thay
thế cây con, (c)--(d) đột biến điểm, (e)--(f) đột biến rút gọn.}
\label{fig:gp-mutation}
\end{figure}

Tóm lại, chương này đã trình bày đầy đủ các nền tảng lý thuyết của GA
và GP. Trên cơ sở đó, Chương 2 sẽ trình bày việc áp dụng GA cho ba
nhóm bài toán đầu tiên -- tối ưu không ràng buộc, tối ưu có ràng buộc,
và bài toán người du lịch -- cùng với việc áp dụng GP cho bài toán hồi
quy symbolic trên dữ liệu quan trắc thời tiết thực tế, dự đoán nhiệt độ
điểm sương từ nhiệt độ không khí và độ ẩm tương đối. Kết quả thực nghiệm
của ba nhóm bài toán GA sẽ được đối
chiếu với một mốc so sánh tương ứng: giá trị cực tiểu toàn cục đã biết
của các hàm benchmark cho bài toán không ràng buộc, nghiệm tối ưu công
bố của bộ benchmark CEC 2006 cho bài toán có ràng buộc, và nghiệm tối ưu
công bố của TSPLIB95 cho bài toán người du lịch. Riêng kết quả của GP
cho bài toán hồi quy symbolic sẽ được đối chiếu với công thức Magnus --
quan hệ vật lý đúng đã biết giữa ba đại lượng -- nhằm đánh giá mức độ
GP tái khám phá được cấu trúc thật của quan hệ này.
